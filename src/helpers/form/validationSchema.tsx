...
export const validationSchema = (field: string, value: any) => {
  let isValid = true;
  if (field === 'email') {
    isValid = regexEmail.test(value);
    return isValid ? null : '不正なメールアドレスです。';
  }
  if (field === 'password') {
    isValid = regexPassword.test(value);
    return isValid ? null : 'パスワードは8文字以上の英数字で入力してください';
  }
  if (field === 'name') {
    isValid = value.length < 100;
    return isValid ? null : 'ユーザー名が長すぎます。';
  }
